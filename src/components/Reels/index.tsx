import { useRef, useState } from "react";
import { Icon } from "../Icon";
import { Like } from "../Like";
import { List } from "../List";
import { Modal, type modalHandles } from "../Modal";
import { Comments, type commentsProps } from "./Comments/Comments";
import { Interaction } from "./Interaction";
import { MoreActions } from "./MoreActions";
import { SendForFriend } from "./SendForFriend";
import { VideoInfo } from "./videoInfo";
import type { tagProps } from "./videoInfo/Tag";
import { VideoSource } from "./videoSource/VideoSource";

interface reelsProps extends commentsProps {
	src: string;
	tags?: tagProps[];
	videoInfo: {
		userName: string;
		likes: number;
		photoSource: string;
		isFollowingThisUser: boolean;
		musicPhotoAlbumSource?: string;
	};
}

export default function Reels({
	src = "",
	videoInfo: {
		userName,
		likes,
		photoSource,
		isFollowingThisUser,
		musicPhotoAlbumSource,
	},
	comments,
	tags,
}: reelsProps) {
	const modalRef = useRef<modalHandles>(null);
	const [modalContent, setModalContent] = useState<React.ReactNode | null>(
		<Comments comments={comments} />,
	);

	const changeModalContent = (content: React.ReactNode | null) => {
		setModalContent(content);
		modalRef.current?.openModal();
	};

	return (
		<div className="relative h-full snap-center">
			<Modal
				ref={modalRef}
				className="bottom-0 right-1/2 rounded-t-2xl min-h-[65%] max-h-[90%] w-full bg-zinc-800"
				resizeble={modalContent !== null}
			>
				{modalContent}
			</Modal>

			<VideoSource src={src} />

			<div className="absolute pt-2 pb-5 px-3 inset-0 h-full w-full grid grid-cols-5 grid-rows-2">
				<header className="flex items-center justify-end h-fit col-span-full text-3xl font-bold text-zinc-100">
					<Icon
						name="camera"
						size={30}
						strokeWidth={1.75}
						className="text-zinc-200"
					/>
				</header>

				<div className="flex flex-col justify-end items-start gap-1.5 text-zinc-50 col-span-4">
					<VideoInfo
						userInfo={{
							userPhoto: photoSource,
							userName: userName,
						}}
						tags={tags}
						following={isFollowingThisUser}
					/>
				</div>

				<List className="justify-end items-center ml-auto gap-6 text-zinc-50 col-span-1 select-none">
					<Interaction>
						<Like likes={likes} size={30} />
					</Interaction>
					<Interaction text="441">
						<Icon
							name="message-circle"
							size={30}
							strokeWidth={1.5}
							onClick={() =>
								changeModalContent(<Comments comments={comments} />)
							}
						/>
					</Interaction>
					<Interaction text="73.3k">
						<Icon
							name="send"
							size={30}
							strokeWidth={1.5}
							onClick={() => changeModalContent(<SendForFriend />)}
						/>
					</Interaction>
					<Interaction>
						<Icon
							name="ellipsis-vertical"
							size={27}
							strokeWidth={1.5}
							onClick={() => changeModalContent(<MoreActions />)}
						/>
					</Interaction>
					<Interaction>
						<img
							src={musicPhotoAlbumSource ?? photoSource}
							alt={userName}
							height={"25px"}
							width={"25px"}
							className="object-cover aspect-square object-top rounded-md outline outline-2 outline-zinc-100"
						/>
					</Interaction>
				</List>
			</div>
		</div>
	);
}
